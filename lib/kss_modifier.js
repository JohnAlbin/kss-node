...
module.exports = KssModifier = function (data) {
	...
	this.data.example = this.data.example || '';
	this.init();
};
...
KssModifier.prototype.example = function() {
	if (!(this.data.section && this.data.section.example)) {
		return false;
	}

	return (this.data.section.example() || '');
};
