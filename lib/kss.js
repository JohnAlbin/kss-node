...
var walk = require('./walk.js'),
	...
	traverse, parse, parseChunk, checkReference, findBlocks, processBlock,
...
parseChunk = function(data, input, options) {
	...
	for (i = 0; i < l; i += 1) {
		...
		currSection = processBlock(paragraphs, options, currSection, 'markup', 'Markup');

		// Before anything else, process the "example" paragraph.
		// It's labelled so can be found right away and then removed
		currSection = processBlock(paragraphs, options, currSection, 'example', 'Example');
...
/**
 * Checks if there is any special blocks listed in the comment block and removes it from the original array.
 * @param  {Array}  paragraphs  An array of the paragraphs in a single block
 * @param  {Object} options     The options object passed on from the initial functions
 * @param  {Object} sectionData The original data object of a section.
 * @return {Object} A new data object for the section.
 */
processBlock = function(paragraphs, options, sectionData, blockName, blockTag) {
	var removeIndex = 'nope';

	paragraphs.map(function(paragraph, index) {
		if (hasPrefix(paragraph, options, blockTag)) {
			sectionData[blockName] = paragraph.replace(/^\s*?[a-z ]+\:\s+?/i, '');
			paragraph = '';
			removeIndex = index;
		}
		return paragraph;
